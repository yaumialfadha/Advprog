...
- [ ] Make at least 7 commit that contains your progress in completing
the tutorial
...
for Model-View-Controller Pattern
    - Explain what is Model-View-Controller Pattern in 1 paragraph
...
Model-View-Controller (MVC) merupakan sebuah software architectural pattern yang melibatkan tiga konsep atau object
dalam sebuah aplikasi, yaitu Model, View, dan Controller. Ketiga konsep tersebut terpisahkan oleh batasan-batasan
abstrak yang menyebabkan ketiganya tidak saling bergantung antar satu sama lain (dependent) namun dapat saling
bekerjasama. Model merupakan sebuah object yang menyimpan data dan main business logic yang dapat digunakan untuk
memanipulasi data tersebut. View merupakan sebuah representasi (seringkali secara visual) dari model-model yang
terasosiasi dengannya. View terpasang oleh satu model dan mendapatkan data yang dibutuhkan, kemudian menampilkan/
mempresentasikannya untuk user. Dalam sebuah web application, view biasanya berupa template HTML yang menampilkan
hasil dari data yang diproses/dimanipulasi oleh sebuah model. Sementara itu, Controller merupakan sebuah object yang
menjembatani (mediator) antara Model dan View. Sebuah object Controller dapat berkomunikasi dengan Model dan
View untuk melakukan aksi tertentu, misalnya Controller menerima input dari user kemudian memprosesnya untuk
meng-update Model.

MVC In Spring Framework

Model
Model pada Spring Framework merupakan object yang mengenkapsulasi data dari aplikasi dan biasanya direpresentasikan
sebagai POJO (Plain Old Java Object) yang tidak memiliki ketergantungan dengan class lain. Contoh implementasi
spesifiknya dalam Spring adalah interface Model.
Controller
Controller dapat berupa sebuah class yang di-bind dengan anotasi @Controller. Sebuah class yang di-bind dengan
anotasi tersebut akan dianggap sebagai sebuah class Controller. Varian lainnya dari Controller adalah anotasi
@RestController yang biasa digunakan sebagai Controller untuk REST web application.
View
Tergantung dengan tipe aplikasi yang dibuat, implementasi View bisa berbeda-beda. Misalnya untuk desktop app, View
dapat merupakan implementasi dari salah satu Java GUI Framework. Sedangkan untuk web app, View dapat berupa HTML +
template engine untuk Spring seperti JSP (Java Server Pages), Thymeleaf, FreeMarker, Groovy, Jade4j, dll.
